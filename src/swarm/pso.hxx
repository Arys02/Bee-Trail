  ...
  template <typename FF>
    void Pso<FF>::evaluate()
    {
      auto body = [=] (size_t i)
      {
        ...
        Vector2 local_best = list_particle_get().at(i)->best_pt_get();
        Vector2 current_pos = list_particle_get().at(i)->pos_get();

        if (fit_fun_(current_pos) <
            fit_fun_(local_best))
        {
          list_particle_get().at(i)->best_pt_set(current_pos);
        }

        /* Update global best if current particule is better */
        local_best = list_particle_get().at(i)->best_pt_get();
